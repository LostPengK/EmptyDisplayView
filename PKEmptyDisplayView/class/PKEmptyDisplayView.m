...
@implementation PKEmptyDisplayView

+(void)load{
    PKEmptyDisplayViewStateConfig *config = [PKEmptyDisplayViewStateConfig shareInstance];
    [config addState:EmptyStateNoData config:@{titleTextKey:@"暂无数据",
                                               detailTextKey:@"",
                                               bundleiImageNameKey:@"pk_empty"}];
    [config addState:EmptyStateNoNetWork config:@{titleTextKey:@"网络已经断开链接",
                                               detailTextKey:@"请点击重试",
                                               bundleiImageNameKey:@"pk_empty_nonetwork"}];
    [config addState:EmptyStateDefault config:@{titleTextKey:@"暂无数据",
                                               detailTextKey:@"",
                                               bundleiImageNameKey:@"pk_empty"}];
}
...
-(void)setState:(EmptyState)state{
    ...
    NSDictionary *stateConfig = [PKEmptyDisplayViewStateConfig  configForState:state];
    if (!stateConfig) {
        return;
    }
    
    self.titleText = stateConfig[titleTextKey];
    self.detailText = stateConfig[detailTextKey];
    if (stateConfig[imageNameKey]) {
        self.imageName = stateConfig[imageNameKey];
    }
    if (stateConfig[bundleiImageNameKey]) {
        self.emptyImageV.image = [NSBundle pk_bundleImageForName:stateConfig[bundleiImageNameKey]];
    }

...
-(void)setEmptyView:(PKEmptyDisplayView *)emptyView{
    objc_setAssociatedObject(self, &EmptyViewKey, emptyView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(PKEmptyDisplayView *)emptyView{
    PKEmptyDisplayView *v = objc_getAssociatedObject(self, &EmptyViewKey);
    
    if (v) {
        return v;
    }
    
    v = [[PKEmptyDisplayView alloc]init];
